...
func main() {
	...
	logger.Infof("initializing candle buffer")
...
func initializeAndRegisterApps(ctx context.Context, logger *log.Logger, dbInstance *db.DB, configs *Configs) {
	var err error
	_, err = posts.NewApp(ctx, logger, configs.PostsApp, dbInstance, nil)
	if err != nil {
		logger.WithError(err).Panicf("failed to initiate posts app")
	}
